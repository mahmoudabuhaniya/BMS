...
class BeneficiaryViewSet(viewsets.ModelViewSet):
    queryset = Beneficiary.all_objects.all().order_by('-created_at')
    serializer_class = BeneficiarySerializer
    permission_classes = [IsAuthenticated]

    filter_backends = [DjangoFilterBackend]
    filterset_fields = ['id', 'record_id', 'InForm_ID' ,'InstanceID' ,'IP_Name' ,'Sector' ,'Indicator' ,'Date' ,'Name' ,'ID_Number' ,'Parent_ID' ,'Spouse_ID' ,'Phone_Number' ,'Date_of_Birth' ,'Age' ,'Gender' ,'Governorate' ,'Municipality' ,'Neighborhood' ,'Site_Name' ,'Disability_Status' ,'created_at' ,'Submission_Time' ,'Deleted' ,'deleted_at' ,'undeleted_at' ,'Household_ID']
...
class ReportView(APIView):
    ...
    def get(self, request):
        # Example: counts by first letter of name
        queryset = Beneficiary.objects.all()
        data = {}
        for b in queryset:
            key = b.name[0].upper()
            data[key] = data.get(key, 0) + 1
        response = [{"category": k, "count": v} for k, v in data.items()]
