...
# 🔹 InForm API constants
'''
INFORM_API_URL = "https://data.inform.unicef.org/unicefstateofpalestine/submission"
INFORM_TOKEN = "ef5a72dad573916ed3a2289e51ca4f1b8ced5c88"  # replace with real one


def check_duplicate(request):
    """Check duplicate by ID number in local DB"""
    id_number = request.GET.get("Id_Number")
    if not id_number:
        return JsonResponse({"exists": False})

    matches = Beneficiary.all_objects.filter(ID_Number=id_number)
    if matches.exists():
        return JsonResponse({
            "exists": True,
            "count": matches.count(),
            "duplicates": list(matches.values("pk", "IP_Name", "Name", "ID_Number", "Governorate", "Date")),
        })
    return JsonResponse({"exists": False})
...
def add_beneficiary(request):
    ...
    required_fields = ["ID_Number", "Name", "IP_Name", "Sector"]
    ...
    if missing:
        ...
        return redirect("beneficiary_add")
