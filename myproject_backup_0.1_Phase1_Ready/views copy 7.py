            ...
            for token in tokens:
                ...
                while True:
                    ...
                    for idx, item in enumerate(data, start=1):
                        ...
                        if Beneficiary.objects.filter(record_id=InForm_ID).exists():
                            duplicate_count += 1
                            continue
...
def inform_data_view(request):
    ...
    # --- Filtering ---
    filter_fields = ['IP_Name','Sector','Indicator','Date','Name','ID_Number','Phone_Number',
                     'Date_of_Birth','Age','Gender','Governorate']
...
def find_duplicates(request):
    ...
    for g in duplicate_groups:
        ...
        records = list(Beneficiary.objects.filter(ID_Number=ID_Number).order_by("id"))
        groups.append({
            "ID_Number": ID_Number,
            "count": g["count"],
            "records": records,
...
def inform_details(request, record_id):
    record = get_object_or_404(Beneficiary, record_id=record_id)
    return render(request, "myproject/inform_details.html", {"record": record})
