            ...
            for token in tokens:
                ...
                while True:
                    ...
                    if response.status_code != 200:
                        progress_status = {
                            "stage": f"⚠️ Failed to fetch data for form {token.form_id} (status {response.status_code})",
                            "progress": 100,
                            "completed": True,
                        }
                        break
                    ...
                    for idx, item in enumerate(data, start=1):
                        ...
                        progress_status = {
                            "stage": f"IP: {get_val('IP_Name')} — Page {page}, Record {idx}/{len(data)}",
                            "progress": min(99, int(((idx + (page - 1) * page_size) / ((page * page_size) + 1)) * 100)),
                            "new_count": new_count,
                        ...
                        uuid = f"uuid:{get_val('_uuid')}"
                        if Beneficiary.all_objects.filter(InstanceID=uuid).exists():
                            duplicate_count += 1
                            continue
                        ...
                        new_count += 1
                        print(f"Imported record {uuid}")
                        print(f"Total fetched: {total_fetched}, New: {new_count}, Duplicates: {duplicate_count}")
                        print(f"Progress: {progress_status['progress']}%")
                        print(get_val("Name"))
                    page += 1
            print(f"Imported record {uuid}")
            print(f"Total fetched: {total_fetched}, New: {new_count}, Duplicates: {duplicate_count}")
